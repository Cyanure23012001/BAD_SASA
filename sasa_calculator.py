...
def parse_pdb_file(pdb_file):
    """
    Parses a PDB file and extracts atomic and residue information.

    Args:
        pdb_file (str): Path to the PDB file.

    Returns:
        np.ndarray: Array containing atomic data.
    """
    data = []
    residues = {}
...
def calc_points(center, radius, n):
    """
    Calculates points on the surface of a sphere centered at 'center' with radius 'radius'.
    ...
    Returns:
        list: Points on the sphere surface.
    """
    points = []
    N = n
...
def is_accessible(point, current_atom, other_atoms):
    """
    Determines if a point is accessible, i.e., not within the radius of any other atom.

    Args:
        point (list): The point to check.
        current_atom (list): The current atom being considered.
        other_atoms (np.ndarray): Array of other atoms.
...
def calculate_sasa(atoms, num_points=100):
    """
    Calculates the Solvent Accessible Surface Area (SASA) for a set of atoms.
...
def calculate_sasa_for_residue(atoms, residue_name, residue_seq_num, num_points=100):
    """
    Calculates the SASA for a specific residue.
    ...
    Returns:
        float: SASA of the specified residue.
    """
...
def calculate_and_write_sasa_for_all_residues(input_pdb, output_pdb, atoms, num_points=100):
    """
    Calculates SASA for all residues in a protein and writes the results to a new PDB file.
...
def write_sasa_to_pdb(input_pdb, output_pdb, sasa_values):
    """
    Writes SASA values to the B-factor column of a PDB file.

    Args:
        input_pdb (str): Path to the input PDB file.
        output_pdb (str): Path to the output PDB file.
        sasa_values (dict): Dictionary of SASA values keyed by residue name and sequence number.
    """
...
def plot_atom_points_plotly(atoms, num_points=100):
    """
    Creates a 3D plot of atoms using Plotly.

    Args:
        atoms (np.ndarray): Array of atoms.
        num_points (int): Number of points to use for each atom's surface.
    """
...
if __name__ == "__main__":
    input_pdb_file = "small_h.pdb"  
    output_pdb_file = "output_with_sasa.pdb"  
